...
import config
import utils
from deepseek_client import chat, chat_complete
from prompts import (
...
def _extract(lid, text, reg):
    existing = [e["title"] for e in reg.values()]
    raw, _ = chat(
        [
            {"role": "system", "content": CONCEPT_EXTRACT_SYSTEM.replace("{course}", config.COURSE_NAME)},
            {"role": "user", "content": build_concept_extract_user(config.COURSE_NAME, lid, text, existing)},
        ],
        temperature=config.TEMP_CONCEPTS,
        json_mode=True,
        tag=f"concept-extract:{lid}",
    )
    data = utils.parse_json_loose(raw)
